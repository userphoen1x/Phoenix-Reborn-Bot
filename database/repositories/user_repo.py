...
class UserRepository:
    ...
    async def get_all_users_for_roles(self) -> List[Dict[str, Any]]:
        cursor = await self.db.execute("""
            SELECT u.user_id, u.bs_tag as tag, u.player_name as name, 
                   p.game_role, p.role_status, p.full_name as tg_name, u.club_name
            FROM users u
            JOIN tg_profiles p ON u.user_id = p.user_id
        """)
        rows = await cursor.fetchall()
        return [{"user_id": r[0], "tag": r[1], "name": r[2], "game_role": r[3], "role_status": r[4], "tg_name": r[5], "club_name": r[6]} for r in rows]
    ...
    async def register_user(self, user_id: int, tag: str, player_name: str, tg_name: str):
        ...
        if await cursor.fetchone():
            await self.db.execute("UPDATE tg_profiles SET full_name = ? WHERE user_id = ?", (tg_name, user_id))
        else:
            await self.db.execute("INSERT INTO tg_profiles (user_id, full_name, game_role, role_status) VALUES (?, ?, 'Гость', 'Одобрен')", (user_id, tg_name))

        cursor2 = await self.db.execute("SELECT user_id FROM users WHERE user_id = ?", (user_id,))
        if await cursor2.fetchone():
            await self.db.execute("UPDATE users SET bs_tag = ?, player_name = ? WHERE user_id = ?", (tag, player_name, user_id))
        else:
    ...
    async def get_tag_to_tg_map(self) -> Dict[str, Dict[str, Any]]:
        ...
        """)
        rows = await cursor.fetchall()
        return {r[0]: {"id": r[1], "name": r[2]} for r in rows if r[0]}

    async def get_all_registered_users(self) -> List[tuple]:
        cursor = await self.db.execute("""
            SELECT p.full_name, u.bs_tag, u.player_name
            FROM tg_profiles p
            JOIN users u ON p.user_id = u.user_id
        """)
        return await cursor.fetchall()

    async def unlink_user_tag(self, target_name: str) -> bool:
        cursor = await self.db.execute("SELECT user_id FROM tg_profiles WHERE full_name = ?", (target_name,))
        row = await cursor.fetchone()
        if not row:
            cursor = await self.db.execute("SELECT user_id FROM tg_profiles WHERE full_name = ?", (target_name.replace("@", ""),))
            row = await cursor.fetchone()
        if row:
            user_id = row[0]
            await self.db.execute("DELETE FROM users WHERE user_id = ?", (user_id,))
            await self.db.execute("UPDATE tg_profiles SET game_role = 'Гость', role_status = 'Одобрен' WHERE user_id = ?", (user_id,))
            await self.db.commit()
            return True
        return False