...
from utils.brawl_api import check_player, update_api_key, check_api_connection, get_all_club_members
...
async def process_tag_input(message: Message, state: FSMContext, bot: Bot):
    ...
    if result["success"] and result["status"] == "member":
        ...
        if members:
            for m in members:
                if m["tag"] == clean_tag:
                    player_role = m.get("role", "member")
                    break

        role_ru = ""
        if player_role == "president":
            role_ru = "Президента"
        elif player_role == "vicePresident":
            role_ru = "Вице-Президента"
