...
ROLE_SYMBOLS = {
    "Главарь": "👑", "Программист": "🧑🏻‍💻", "Президент": "🌟", "Вице-президент": "⭐",
    "Ветеран": "🎖", "Участник": "👤", "Гость": "🗣️"
}
...
def get_rank_name(val: int):
    ranks = {
        1: "🥉 Бронза 1", 2: "🥉 Бронза 2", 3: "🥉 Бронза 3", 4: "🥈 Серебро 1", 5: "🥈 Серебро 2", 6: "🥈 Серебро 3",
...
async def get_roles_bulk(uids: list, user_repo: UserRepository):
    async def safe_get(uid):
        ...
        try:
            res = await user_repo.get_user_role(uid)
...
async def cmd_top_trigger(message: Message, user_repo: UserRepository, chat_repo: ChatRepository,
    ...
    if is_push_direct:
        ...
        roles = await get_roles_bulk(uids, user_repo)
...
async def process_top_callbacks(callback: CallbackQuery, callback_data: TopCb, user_repo: UserRepository,
    ...
    if callback.from_user.id != callback_data.uid:
        return await callback.answer("❌ Не твое меню", show_alert=True)
    ...
    if act == "main":
        ...
        if not members:
            return await callback.message.edit_text("❌ Ошибка", reply_markup=back,
                                                    link_preview_options=LinkPreviewOptions(is_disabled=True))
        ...
        try:
            ...
            if not live_members:
                return await callback.message.edit_text("❌ Ошибка API",
                                                        reply_markup=kb_timeframe("cups_gain", "cat", uid, c),
                                                        link_preview_options=LinkPreviewOptions(is_disabled=True))
