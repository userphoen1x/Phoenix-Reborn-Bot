...
def kb_main_top(uid: int, c: str):
    ...
    buttons.append([InlineKeyboardButton(text="⚔️ Победы", callback_data=TopCb(act="wins", uid=uid, c=c).pack()),
...
async def cmd_top_trigger(message: Message, user_repo: UserRepository, chat_repo: ChatRepository,
                          eco_repo: EconomyRepository, brawl_client: BrawlAPIClient):
    ...
    if any(word in args_lower for word in
        ...
        is_push_direct = True; push_days = 7; push_title = "за неделю"
    elif any(
        word in args_lower for word in ["пушеров месяца", "пушеров месяц", "пушеры месяц", "пуш месяц", "рост месяц"]):
        is_push_direct = True; push_days = 30; push_title = "за месяц"
    ...
    if is_push_direct:
        ...
        try:
            ...
            uids = [tg_map.get(tag_str, {}).get("id") if isinstance(tg_map.get(tag_str), dict) else None for
                    _, _, tag_str in results]
            ...
            back = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ К меню топов",
                                                                               callback_data=TopCb(act="main", uid=uid,
                                                                                                   c="ALL").pack())]])
            ...
            await sent_msg.edit_text(f"❌ Ошибка вычислений: {str(e)}",
                                     link_preview_options=LinkPreviewOptions(is_disabled=True))
            asyncio.create_task(delete_later(sent_msg))
        return
    ...
    if args_str in msg_triggers:
        ...
        try:
            ...
            if not members:
                ...
                tg_map = await user_repo.get_tag_to_tg_map()
                top_10 = members[:10]
                uids = [tg_map.get(m["tag"], {}).get("id") if isinstance(tg_map.get(m["tag"]), dict) else None for m in
                        top_10]
                syms_list = await get_roles_bulk(uids, user_repo)
        ...
        try:
            ...
            tg_map = await user_repo.get_tag_to_tg_map()
            if not members:
                await sent_msg.edit_text("❌ Ошибка загрузки.",
                                         link_preview_options=LinkPreviewOptions(is_disabled=True))
            else:
...
async def process_top_callbacks(callback: CallbackQuery, callback_data: TopCb, user_repo: UserRepository,
                                chat_repo: ChatRepository, eco_repo: EconomyRepository, brawl_client: BrawlAPIClient):
    ...
    if act == "main":
        ...
        try:
            ...
            await callback.message.edit_text(f"❌ Ошибка вычислений: {str(e)}", reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="⬅️ Назад", callback_data=TopCb(act="cat", uid=uid, c="ALL").pack())]]),
                                             link_preview_options=LinkPreviewOptions(is_disabled=True))
    elif act == "cups_cur":
                ...
                await callback.message.edit_text(f"❌ Ошибка загрузки.\nДетали: {err}" if err else "❌ Ошибка загрузки.",
                                                 reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                                     [InlineKeyboardButton(text="⬅️ Назад",
                                                                           callback_data=TopCb(act="cat", uid=uid,
                                                                                               c=c).pack())]]),
                                                 link_preview_options=LinkPreviewOptions(is_disabled=True))
                return
            members.sort(key=lambda x: x.get("trophies", 0), reverse=True)
        ...
        try:
            members, err = await brawl_client.get_live_club_detailed_stats(c)
            tg_map = await user_repo.get_tag_to_tg_map()
            if not members: return await callback.message.edit_text("❌ Ошибка загрузки", reply_markup=back,
                                                                    link_preview_options=LinkPreviewOptions(
                                                                        is_disabled=True))
            if act == "ranks_curr":
                sort_key = lambda x: (x.get("ranked_curr_rank", 0), x.get("ranked_curr_elo", 0)); title = "🎖 Ранкед"
            elif act == "wins_3v3":
                sort_key = lambda x: x.get("wins_3v3", 0); title = "⚔️ 3 на 3"
            elif act == "wins_sd_solo":
                sort_key = lambda x: x.get("solo_wins", 0); title = "👤 Соло"
            elif act == "wins_sd_duo":
                sort_key = lambda x: x.get("duo_wins", 0); title = "👥 Дуо"
            ...
            for i, m in enumerate(top_10):
                ...
                if act == "ranks_curr":
                    ...
                    txt += f"{place} {sym} {name_link}: {sort_key(m)}\n"
            if err: txt += f"\nОшибки: {err}"
            await callback.message.edit_text(txt, reply_markup=back,
                                             link_preview_options=LinkPreviewOptions(is_disabled=True))
        except Exception as e:
            logging.exception("Ошибка в коллбэках побед/ранкеда:")
            await callback.message.edit_text(f"❌ Ошибка вычислений: {str(e)}", reply_markup=back,
                                             link_preview_options=LinkPreviewOptions(is_disabled=True))
        ...
        try:
            ...
            await callback.message.edit_text(f"❌ Ошибка вычислений: {str(e)}",
                                             reply_markup=kb_timeframe("msg", "cat", uid, c),
                                             link_preview_options=LinkPreviewOptions(is_disabled=True))
