...
async def cb_casino_handler(callback: CallbackQuery, callback_data: CasinoCb, casino_service: CasinoService):
    ...
    if act == "play":
        try:
            ...
            if balance < bet: return await callback.answer("Недостаточно средств!", show_alert=True)
            if game == "bj":
                await start_blackjack(callback, user_id, bet, casino_service)
                return
            await cleanup_old_game(callback.bot, callback.message.chat.id, user_id)
            try:
                await callback.message.delete()
...
async def start_blackjack(callback: CallbackQuery, user_id: int, bet: int, casino_service: CasinoService):
    ...
    # Теперь мы НЕ сохраняем casino_service в глобальный словарь!
    ...
    if pval == 21:
        # Передаем casino_service дальше по цепочке
        await finish_blackjack(callback, user_id, "bj", casino_service)
...
async def cb_bj_handler(callback: CallbackQuery, callback_data: BjCb,
                        casino_service: CasinoService):  # Получаем service из Middleware
    user_id = callback.from_user.id
...
async def start_saper_game(chat_id: int, user_id: int, user_name: str, bet_str: str, diff: str,
    ...
    try:
        ...
        # Мы НЕ сохраняем casino_service
        SAPER_GAMES[user_id] = {"chat_id": chat_id, "name": user_name, "bet": bet, "diff": diff, "grid": grid,
                                "clicked": [], "mult": 1.0}
...
async def cb_saper_play(callback: CallbackQuery, callback_data: SaperCb,
    ...
    if not game or game["chat_id"] != callback.message.chat.id: return await callback.answer(
