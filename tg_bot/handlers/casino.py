...
async def start_blackjack(callback: CallbackQuery, user_id: int, bet: int, casino_service: CasinoService):
    ...
    BJ_GAMES[user_id] = {"bet": bet, "deck": deck, "player_hand": player_hand, "dealer_hand": dealer_hand, "service": casino_service}
...
async def cb_bj_handler(callback: CallbackQuery, callback_data: BjCb):
    ...
    if act == "hit":
        ...
        pval = calc_hand(game["player_hand"])
        if pval > 21: await finish_blackjack(callback, user_id, "bust")
        elif pval == 21: await finish_blackjack(callback, user_id, "stand")
...
async def start_saper_game(chat_id: int, user_id: int, user_name: str, bet_str: str, diff: str, casino_service: CasinoService, bot_msg=None):
    ...
    try:
        ...
        SAPER_GAMES[user_id] = {"chat_id": chat_id, "name": user_name, "bet": bet, "diff": diff, "grid": grid, "clicked": [], "mult": 1.0, "service": casino_service}
