...
class AiService:
    ...
    async def generate_response(self, chat_id: int, user_name: str, current_msg: str, bot_id: int, history: list,
                                media_data: tuple = None) -> str:
        mode = await self.chat_repo.get_chat_mode(chat_id)
        system_prompt = self.prompts.get(mode, self.prompts[
        ...
        if media_data:
            ...
            if m_type == "photo":
                ...
                messages = [
                    ...
                    {"role": "user",
                     "content": [{"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{data}"}},
                                 {"type": "text",
                                  "text": f"--- ТЕКУЩИЙ ЗАПРОС: {caption if caption else 'что на этой картинке?'} ---"}]}
                ]
                raw_response = await asyncio.get_event_loop().run_in_executor(None, self.groq_client.ask_vision,
                                                                              messages)
                ...
                messages_for_ai.append(
                    {"role": "user", "content": f"--- {user_name} прислал {label}, вот транскрипт: {data} ---"})
                raw_response = await asyncio.get_event_loop().run_in_executor(None, self.groq_client.ask,
                                                                              messages_for_ai)
                return re.sub(r"^[^:：]{1,30}[:：]\s*", "", raw_response, flags=re.IGNORECASE).strip()
        ...
        for i, (uid, full_name, text) in enumerate(history):
            role_str = "assistant" if uid == bot_id else "user"
            content = f"--- ТЕКУЩИЙ ЗАПРОС ОТ {full_name}: {text} ---" if i == len(
                history) - 1 else f"{full_name}: {text}"
        ...
        raw_response = await asyncio.get_event_loop().run_in_executor(None, self.groq_client.ask, messages_for_ai)
        return re.sub(r"^[^:：]{1,30}[:：]\s*", "", raw_response, flags=re.IGNORECASE).strip()

    async def transcribe_audio(self, audio_bytes: bytes, filename: str) -> str:
        return await asyncio.get_event_loop().run_in_executor(None, self.groq_client.transcribe_audio, audio_bytes,
                                                              filename)

    async def generate_voice(self, text: str) -> bytes | None:
        return await asyncio.get_event_loop().run_in_executor(None, self.groq_client.generate_speech, text)