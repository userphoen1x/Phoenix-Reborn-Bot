...
async def check_api_connection():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as resp:
                if resp.status == 200: return True, "✅ Соединение установлено (200 OK)."
                elif resp.status == 403: return False, "❌ Ошибка 403 (Forbidden): Обновите IP в ключе."
                elif resp.status == 429: return False, "⚠️ Ошибка 429: Временный бан за спам."
        ...
        try:
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    return {"success": True, "status": "member" if club_data.get("tag") in CLAN_TAGS else "not_member", "name": data.get("name", "Неизвестно"), "club_name": club_data.get("name", "Phoenix Reborn")}
                return {"success": False, "error": "api_error"}
        except: return {"success": False, "error": "connection_error"}
...
async def get_live_club_detailed_stats(specific_club: str = None):
    ...
    async def fetch_for_member(m):
        async with sem:
            ...
            else: m.update({"solo_wins": 0, "duo_wins": 0, "wins_3v3": 0, "exp_level": 0, "highest_trophies": 0})
            return m
