...
async def collect_daily_stats():
    ...
    async with aiosqlite.connect(settings.DB_PATH) as db:
        ...
        for m in members:
            ...
            if stats:
                await chat_repo.save_snapshot(tag=tag, name=name, dt=today, trophies=stats["trophies"], solo=stats["solo_wins"], duo=stats["duo_wins"], wins3v3=stats["wins_3v3"], rank_c=stats.get("ranked_curr_rank", 0), rank_h=0)
...
async def check_roles(bot: Bot):
    ...
    async with aiosqlite.connect(settings.DB_PATH) as db:
        ...
        for user in db_users:
            ...
            if db_role in ["Главарь", "Программист"]: continue
            if tag not in api_roles:
                if db_role != "Гость":
                    await user_repo.set_user_role(u_id, "Гость", "Одобрен")
                    if settings.TARGET_CHAT_ID:
                        try: await bot.promote_chat_member(chat_id=settings.TARGET_CHAT_ID, user_id=u_id, can_manage_chat=False)
                        except: pass
            else:
                ...
                if api_role in ["Участник", "Ветеран"]:
                    ...
                    if settings.FOUNDER_ID:
                        ...
                        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Да", callback_data=f"role_approve:{u_id}:{role_eng}")], [InlineKeyboardButton(text="Нет", callback_data=f"role_reject:{u_id}")]])
                        try: await bot.send_message(settings.FOUNDER_ID, f"Пользователь ({u_id}) получил {api_role} в игре. Подтверждаете выдачу прав в чате?", reply_markup=kb)
                        except: pass
...
async def backup_database(bot: Bot):
    ...
    try:
        ...
        await bot.send_document(chat_id=int(settings.ADMIN_CHAT_ID), message_thread_id=int(settings.TOPIC_BACKUP), document=db_file, caption=f"📦 <b>Ежедневный бэкап базы данных</b>\n📅 Дата: {date.today().isoformat()}", parse_mode="HTML")
