            ...
            if tag not in api_roles:
                ...
                if api_role in ["Участник", "Ветеран"]:
                    ...
                    if settings.FOUNDER_ID:
                        ...
                        try:
                            await bot.send_message(settings.FOUNDER_ID,
                                                   f"Пользователь ({u_id}) получил {api_role} в игре. Подтверждаете выдачу прав в чате?",
                                                   reply_markup=kb)
                        except:
                            pass
...
async def backup_database(bot: Bot):
    if not settings.ADMIN_CHAT_ID or not settings.TOPIC_BACKUP:
        return
    ...
    try:
        ...
        await bot.send_document(
            ...
            message_thread_id=int(settings.TOPIC_BACKUP),
            document=db_file,
